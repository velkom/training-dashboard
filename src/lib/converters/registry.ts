...
import type { ImportParseNotes, WorkoutConverter } from "./types";
...
export async function convertWithDetection(
  ...
  const { sessions, parseNotes } = await converter.convert(input, userId);
  return { converter, sessions, parseNotes };
}
